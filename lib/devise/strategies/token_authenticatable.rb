module TokenAuthenticable
  class CustomStrategy < Devise::Strategies::Authenticatable
    ...
    def authenticate!
      ...
      if @env['X-AUTH-TOKEN'].present?
        # the returned user object will be saved and serialised into the session
        user = klass.find_by(authentication_token: @env['X-AUTH-TOKEN'])
        success! user
      end
...
Warden::Strategies.add :token_authenticatable, TokenAuthenticable::CustomStrategy
